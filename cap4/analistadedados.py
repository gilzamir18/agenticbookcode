...
# --- Bloco 2: Verificador SQL (fonte de feedback externo) ---
@as_tool
...
async def main():
    ...
    graph = WorkflowGraph()
    graph.add_block(gerador_sql)
    graph.add_block(verificador_sql)
    ...
    print(f"\n{'='*60}")
    print(f"Iterações: {cr.iterations}  |  SQL aprovado: {cr.validated}")
    print(f"\nSQL final gerado:\n{cr.output.response}")

    if cr.validated:
        validador_out = ctx.get_output("verificador_sql")
        print(f"\nResultado da consulta:\n{validador_out.result['feedback']}")
